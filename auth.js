...
(function(){
  ...
  function init(){
    ...
    /* captura o resultado do redirect do Google (se houver) */
    _auth.getRedirectResult().then(function(result){
      if (result && result.user){
        console.log('[GonAuth] getRedirectResult: logado como', result.user.email);
      }
    }).catch(function(err){
      console.error('[GonAuth] getRedirectResult error:', err);
    });
