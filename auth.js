...
(function(){
  ...
  function init(){
    ...
    /* Primeiro tenta capturar o resultado do redirect.
       Só depois disso o onAuthStateChanged vai ter o usuário correto. */
    _auth.getRedirectResult().then(function(result){
      if (result && result.user){
        console.log('[GonAuth] redirect login OK:', result.user.email);
      }
      /* agora registra o listener — neste ponto a sessão já está persistida */
      ...
      console.error('[GonAuth] getRedirectResult error:', err.code, err.message);
      /* mesmo com erro, registra o listener */
      _auth.onAuthStateChanged(function(user){
        console.log('[GonAuth] onAuthStateChanged (após erro):', user ? user.email : 'null');
        _user = user;
        notifyReady();
      });
    });
